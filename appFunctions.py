...
def get_restaurant_details(row):
    ...
    # Create HTML content to display this information
    details_layout = html.Div([
        html.H3(f"{'★' * int(stars)} {name}", className='restaurant-name'),
        html.P(f"Cuisine: {cuisine}", className='restaurant-cuisine'),
        html.P(f"Price Range: {price}", className='restaurant-price'),
        html.P(address, className='restaurant-address'),
        html.P(location, className='restaurant-location'),
        html.A("Visit Website", href=website_url, target='_blank', className='restaurant-website')
...
#                     f"<br>"
#                     f"<a href='{row['url']}' target='_blank' class='link-highlight' style='font-family: Libre Franklin, sans-serif; font-size: 15px; color: {text_color_map[row['stars']]}'>Visit website</a>",
#         axis=1
#     )
#
#     # Overlay restaurant points without adding them to the legend
